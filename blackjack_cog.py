...
class BlackjackCog(commands.Cog):
    ...
    def __init__(self, bot: commands.Bot, use_case: GameUseCase, presenter: DiscordPresenter):
        self.bot = bot
    ...
    async def _waiting_room_timeout(self, channel_id: int, ctx: commands.Context):
        ...
        if game and game.state == GameState.WAITING_FOR_PLAYERS and len(game.players) <= 1:
            ...
            await ctx.send("⏰ Phòng chờ đã bị đóng do không có ai tham gia sau 5 phút.")
    ...
    async def blackjack(self, ctx: commands.Context):
        ...
        if game and game.state in (GameState.WAITING_FOR_PLAYERS, GameState.PLAYERS_TURN, GameState.DEALER_TURN):
            await ctx.send("❌ Đã có một phòng chờ/game đang diễn ra trong kênh này. Hãy kết thúc ván hiện tại trước khi tạo mới.")
            self.logger.warning(f"Channel {ctx.channel.id} đã có game active, không tạo mới.")
            return
        # Nếu không có, tạo phòng chờ mới
        game, joined = self.use_case.join_game(ctx.channel.id, ctx.author.id, ctx.author.display_name)
        self.game_starters[ctx.channel.id] = ctx.author.id
        self.logger.info(f"Tạo phòng chờ mới ở channel {ctx.channel.id} bởi user {ctx.author.id} ({ctx.author.display_name})")
    ...
    async def join(self, ctx: commands.Context):
        ...
        try:
            game, joined = self.use_case.join_game(ctx.channel.id, ctx.author.id, ctx.author.display_name)
            if joined:
                self.logger.info(f"User {ctx.author.id} ({ctx.author.display_name}) join phòng chờ channel {ctx.channel.id}")
                await ctx.send(f"{ctx.author.display_name} đã tham gia ván đấu!")
                embed = self.presenter.create_waiting_embed(game)
                await ctx.send(embed=embed)
                # Nếu có nhiều hơn 1 người chơi, hủy timeout
                if ctx.channel.id in self.waiting_room_timeouts and len(game.players) > 1:
                    self.waiting_room_timeouts[ctx.channel.id].cancel()
                    del self.waiting_room_timeouts[ctx.channel.id]
            else:
                await ctx.send(f"{ctx.author.display_name}, bạn đã ở trong phòng chờ rồi.")
        except RuntimeError as e:
            self.logger.warning(f"User {ctx.author.id} join phòng chờ channel {ctx.channel.id} lỗi: {e}")
    ...
    async def start(self, ctx: commands.Context):
        ...
        if starter != ctx.author.id:
            ...
            self.logger.warning(f"User {ctx.author.id} cố gắng start game ở channel {ctx.channel.id} nhưng không phải starter.")
            return

        game = self.use_case.repo.get_game(ctx.channel.id)
        if not game or not game.players:
            await ctx.send("Không có ai trong phòng chờ để bắt đầu.")
            self.logger.warning(f"Channel {ctx.channel.id} không có ai trong phòng chờ khi start.")
            return
        
        if game.state != GameState.WAITING_FOR_PLAYERS:
            await ctx.send("Ván chơi đã bắt đầu rồi.")
            self.logger.warning(f"Channel {ctx.channel.id} đã start game khi game đã chạy.")
            return

        players_data = {p.id: p.name for p in game.players.values()}
        game = self.use_case.start_new_game(ctx.channel.id, players_data)
        
        self.logger.info(f"Game bắt đầu ở channel {ctx.channel.id} với {len(players_data)} người chơi.")
    ...
    async def end_game_command(self, ctx: commands.Context):
        ...
        if starter == ctx.author.id or ctx.author.guild_permissions.manage_channels:
            ...
            self.logger.info(f"Game ở channel {ctx.channel.id} đã bị kết thúc bởi user {ctx.author.id} ({ctx.author.display_name})")
            # Hủy timeout nếu có
            if ctx.channel.id in self.waiting_room_timeouts:
                self.waiting_room_timeouts[ctx.channel.id].cancel()
                del self.waiting_room_timeouts[ctx.channel.id]
            ...
            self.logger.warning(f"User {ctx.author.id} cố gắng end game ở channel {ctx.channel.id} nhưng không có quyền.")
